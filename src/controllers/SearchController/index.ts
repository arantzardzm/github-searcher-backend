...
export default async function searchController(req: Request, res: Response) {
  try {
    ...
    if (!checkAlphaRegex(selectInput) || !checkAlphaNumRegex(searchInput)) {
      return res.send({ status: 500, message: 'Invalid data provided', data: [] });
    }
    const response = await axios.get(`https://api.github.com/search/${selectInput}?q=${searchInput}&page=1&per_page=100`);
